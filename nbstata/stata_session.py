...
# %% auto 0
__all__ = ['StataSession', 'update_graph_config', 'warn_re_unclosed_comment_block_if_needed']

# %% ../nbs/08_stata_session.ipynb 4
from .misc_utils import print_red
from .config import launch_stata, set_graph_format, set_graph_size, Config
...
def update_graph_config(config: Config):
    graph_format = config.env['graph_format']
    if graph_format == 'pystata':
        graph_format = 'svg'
    set_graph_format(graph_format)
    set_graph_size(config.env['graph_width'], config.env['graph_height'])
